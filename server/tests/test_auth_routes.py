...
async def test_login_valid_by_username_returns_token(app_client):
    """Login with username must succeed."""
    creds = _unique_creds()
    await app_client.post("/auth/signup", json=creds)
...
@pytest.mark.asyncio
async def test_login_by_email_no_longer_authenticates(app_client):
    """Email must NOT authenticate — username-only login."""
    creds = _unique_creds()
    await app_client.post("/auth/signup", json=creds)
    resp = await app_client.post(
        "/auth/login",
...
async def test_login_wrong_password_401_generic(app_client):
    ...
    resp = await app_client.post(
        "/auth/login",
        json={"username": creds["username"], "password": "wrong-password-here"},
    )
    assert resp.status_code == 401
    assert resp.json()["message"] == "Invalid credentials"
...
@pytest.mark.asyncio
async def test_login_unknown_username_401_generic(app_client):
    resp = await app_client.post(
        "/auth/login",
        json={"username": "no_such_user", "password": "anythinghere"},
    )
    assert resp.status_code == 401
    assert resp.json()["message"] == "Invalid credentials"
