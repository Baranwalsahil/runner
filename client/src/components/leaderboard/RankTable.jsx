...
export default function RankTable({ players, currentUserId }) {
  ...
  return (
    <div data-testid="rank-table" className="scifi-panel rounded-xl overflow-hidden">
      ...
      {pageRows.map((p) => {
        ...
        return (
          ...
          >
            ...
            <div className={`col-span-2 text-right font-scifi font-light text-xl ${isMe ? "text-secondary-fixed" : "text-secondary-fixed-dim"} [text-shadow:0_0_12px_rgba(0,219,233,0.35)]`}>
      ...
      <div className="flex items-center justify-between px-md py-md border-t border-secondary-fixed-dim/15">
        <button
          ...
          disabled={page === 1}
          className="font-scifi text-xs uppercase tracking-[0.2em] px-md py-sm rounded-lg border border-secondary-fixed-dim/30 hover:border-secondary-fixed-dim/60 hover:text-secondary-fixed transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        >
          PREV
        </button>
