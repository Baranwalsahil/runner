...
export default function AlertBar({ message, ctaLabel, onCta }) {
  ...
  return (
    ...
    >
      ...
      <span className="font-label-bold text-on-error-container uppercase tracking-widest text-[12px]">
        {message}
      </span>
      {ctaLabel && (
        <button
          onClick={onCta}
          className="ml-base bg-on-error-container text-error-container px-sm py-xs font-label-bold rounded-full hover:brightness-110 transition-all"
        >
          {ctaLabel}
        </button>
      )}
