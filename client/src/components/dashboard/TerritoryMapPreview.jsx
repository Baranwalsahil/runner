...
export default function TerritoryMapPreview({
  ...
  return (
    <div
      ...
      className="lg:col-span-3 scifi-panel rounded-2xl overflow-hidden relative group h-[500px]"
    >
      {showMap ? (
        <MapCanvas
      ...
      {showNoCellsOverlay && (
        ...
        >
          <div className="scifi-panel rounded-xl px-md py-sm text-center font-scifi uppercase tracking-[0.2em] text-sm pointer-events-auto">
            No cells claimed near you
          </div>
        </div>
      )}
      <div className="absolute inset-0 bg-gradient-to-t from-surface via-transparent to-transparent pointer-events-none" />
      <div className="absolute top-md left-md p-sm scifi-panel rounded-xl z-10">
        <div className="flex items-center gap-base">
          <div className="w-2.5 h-2.5 rounded-full bg-secondary-fixed animate-pulse shadow-[0_0_10px_rgba(0,219,233,0.8)]" />
          <span className="font-scifi text-xs uppercase tracking-[0.15em]">{liveLabel}: {liveBattles}</span>
        </div>
      </div>
      <div className="absolute bottom-md right-md flex flex-col gap-base z-10">
        <button
          ...
          className="bg-[rgba(11,15,22,0.8)] backdrop-blur-md p-base rounded-full border border-secondary-fixed-dim/30 hover:border-secondary-fixed-dim hover:text-secondary-fixed transition-all"
        >
          <Icon name="zoom_in" />
        </button>
        <button
